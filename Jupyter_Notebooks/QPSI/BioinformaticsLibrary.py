...
#          MF = MF.merge(full_Enrich, how='left', left_on='native', right_on='native')
          ...
          MF = MF.merge(full_Enrich, how='left', left_on='native', right_on='term_id')
            ...
            full_Enrich['New Name'] = full_Enrich['native'] + ' ' + full_Enrich['name']
            full_Enrich['Comparison'] = full_Enrich['Comparison'].str.replace('_', ' ')
            BP = full_Enrich[full_Enrich['Ontology'] == 'BP']
            CC = full_Enrich[full_Enrich['Ontology'] == 'CC']
            MF = full_Enrich[full_Enrich['Ontology'] == 'MF']
#      return(BP)      
#      BP = BP[BP['p_value_x'] <= adj_Pthresh]
#      CC = CC[CC['p_value_x'] <= adj_Pthresh]
#      MF = MF[MF['p_value_x'] <= adj_Pthresh]
      
      BP = BP[BP['FDR'] <= adj_Pthresh]
      CC = CC[CC['FDR'] <= adj_Pthresh]
      MF = MF[MF['FDR'] <= adj_Pthresh]
      
      BP = BP.drop_duplicates(subset=['New Name', 'Comparison'])
      CC = CC.drop_duplicates(subset=['New Name', 'Comparison'])
      MF = MF.drop_duplicates(subset=['New Name', 'Comparison'])
...
#      plt.rcParams["figure.figsize"] = [1.50, 7.50]
      ...
      sns.scatterplot(
          data=BP, x="Comparison", y="New Name", hue="FDR", 
          size="GeneRatio",
          sizes=(dot_min_size, dot_max_size), hue_norm=hue, legend="brief", palette="vlag_r")
...
#      plt.legend(loc='best')
      ...
      f, ax = plt.subplots(figsize=(int(plot_width), int(plot_height)))
      sns.scatterplot(
          data=CC, x="Comparison", y="New Name", hue="FDR", 
          size="GeneRatio",
          sizes=(dot_min_size, dot_max_size), hue_norm=hue, legend="brief", palette="vlag_r")
      ax.set_ylabel('GO Term', size=14)
#      plt.legend(loc='best')
      ...
      f, ax = plt.subplots(figsize=(int(plot_width), int(plot_height)))
      sns.scatterplot(
          data=MF, x="Comparison", y="New Name", hue="FDR", 
          size="GeneRatio",
          sizes=(dot_min_size, dot_max_size), hue_norm=hue, legend="brief", palette="vlag_r")
      ax.set_ylabel('GO Term', size=14)
 #     plt.legend(loc='best')
      ...
      if sort_list is not None:
          list(reversed(sort_list))
          f, ax = plt.subplots(figsize=(int(plot_width), int(plot_height)))
          sns.scatterplot(
              data=full_Enrich, x="Comparison", y="New Name", hue="FDR", 
              size="GeneRatio",
              sizes=(dot_min_size, dot_max_size), hue_norm=hue, legend="brief", palette="vlag_r")
#          ax.set_ylabel('GO Term', size=14)
     #     plt.legend(loc='best')
          plt.legend(bbox_to_anchor=(1.05, 1))
          plt.title("Full gProfiler Enrichment", size=18)
          plt.xticks(rotation=90)
          plt.setp(ax, yticklabels=sort_list)
#          plt.yticks(sort_list)
          fileName = 'Full_enrich_dotplot' + output_type
          plt.savefig(fileName, bbox_inches='tight')
      else:
          f, ax = plt.subplots(figsize=(int(plot_width), int(plot_height)))
          sns.scatterplot(
...
def Zscore_from_GO_term_list(GO_term_file, count_file,
                             GO_annotation_file, output_name=None,
                             subset_list_1=None,
                             subset_list_2=None):
    if '.csv' in GO_term_file:
        GO_DF = pd.read_csv(GO_term_file)
    elif '.xlsx' in GO_term_file:
        GO_DF = pd.read_excel(GO_term_file)
    else:
        raise ValueError('You did not supply a .xlsx or .csv for GO_term_file')
        
    if '.csv' in count_file:
        avg_counts = pd.read_csv(count_file, index_col=0)
    elif '.xlsx' in count_file:
        avg_counts = pd.read_excel(count_file, index_col=0)
    else:
        raise ValueError('You did not supply a .xlsx or .csv for count_file')
        
    if '.csv' in GO_annotation_file:
        gmt = pd.read_csv(GO_annotation_file)
    elif '.xlsx' in GO_annotation_file:
        gmt = pd.read_excel(GO_annotation_file)
    elif '.tsv' in GO_annotation_file:
        gmt = pd.read_csv(GO_annotation_file, sep='\t')
    else:
        raise ValueError('You did not supply a .xlsx, .csv, or .tsv for GO_annotation_file')
    
    if subset_list_1 is not None:
        if type(subset_list_1) != list:
            raise ValueError('subset_list_1 must be of list format: ["example1","example2"]')
            
    if subset_list_2 is not None:
        if type(subset_list_2) != list:
            raise ValueError('subset_list_2 must be of list format: ["example1","example2"]')
    
    if output_name is None:
        filename = 'Zscore_Output'
    else:
        filename = str(output_name)
            
    GO_rename_dict = dict(zip(GO_DF['GO ID'], GO_DF['GO Name']))
    
    GO_list = GO_DF['GO ID'].unique()
    
    finalDF = pd.DataFrame(columns=avg_counts.columns)
    
    for i in GO_list:
        genes = gmt[gmt['GO_ID']==i]
        gene_list = genes['Gene_id'].unique()
        temp_counts = avg_counts[avg_counts.index.isin(gene_list)].mean()
        GO_name = GO_rename_dict.get(i)
        row_name = i + ': ' + GO_name
        temp_counts.name = row_name
        finalDF = finalDF.append(temp_counts)

    full_filename = 'Full_' + filename + '.csv'

    zscore_final = zscore(finalDF, axis=1, nan_policy='omit')
    zscore_final.to_csv(full_filename)
    
    if subset_list_1 is not None:
        if subset_list_2 is None:
            for i in subset_list_1:
                list_cols = [c for c in finalDF.columns if i in c]
                sub_finalDF = finalDF[list_cols]
                subset_filename = i + '_' + filename + '.csv'
                sub_finalDF = zscore(sub_finalDF, axis=1, nan_policy='omit')
                sub_finalDF.to_csv(subset_filename)
        else:
            for a in subset_list_1:
                list_cols = [c for c in finalDF.columns if a in c]
                sub_finalDF = finalDF[list_cols]          
                for b in subset_list_2:
                    list_cols2 = [c for c in sub_finalDF.columns if b in c]
                    sub2_finalDF = sub_finalDF[list_cols2]
